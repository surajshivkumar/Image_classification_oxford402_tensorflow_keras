...
def predict(image_path, model, top_k):
    ...
    classes = np.argpartition(p, -top_k)[-top_k:] 
    probable_class = classes.tolist()
    probable_class = probable_class[::-1]
    return probabilities, probable_class
...
def main():
    ...
    probs, classes = predict(img_path, model, k)
    print(type(classes))
    probs = probs.tolist()
    ...
    for i in range(len(classes)):
        print('{} Prediction = {} with probabilty {}'.format(i+1, labels[str(classes[i])], probability[i]))
